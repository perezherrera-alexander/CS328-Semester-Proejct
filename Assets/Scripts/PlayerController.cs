...
#pragma warning restore 0414
    ...
    private float maxBeamLength = 5.5f;
    ...
    void LookAtMouse(UnityEngine.Vector2 mouseDirection)
    { // Moved this functionality into FixedUpdate() since it's trivial
    ...
    void FireBeam(UnityEngine.Vector2 mouseDirection)
    {
        ...
        foreach (RaycastHit2D h in hit)
        {
            if (h.collider.gameObject.CompareTag("Wall"))
            {
                ...
                h.collider.gameObject.CompareTag("Enemy")
                || h.collider.gameObject.CompareTag("Goblin")
                || h.collider.gameObject.CompareTag("Boss")
            )
            {
        ...
        if (!hitWall)
        {
            lineRenderer.SetPosition(
                1,
                transform.position + transform.up.normalized * maxBeamLength
            );
        }
        ...
        if (enemiesHit.Count > 0 && timeSinceLastBeam > beamDamageTime)
        {
            foreach (GameObject enemy in enemiesHit)
            {
                if (enemy != null)
                {
                    ...
                    {
                        Debug.Log(
                            "Uh oh, someone didn't add a case for this enemy (Alexander's fault)"
                        );
    ...
    public void Shoot()
    {
        ...
        else
        {
            ...
            float adjustedBulletSpeed =
                bulletSpeed + Random.Range(-randomSpeedRange, randomSpeedRange); // Get random speed deviation
    ...
    public void HandleDash(UnityEngine.Vector2 direction)
